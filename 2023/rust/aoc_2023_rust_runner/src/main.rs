...
fn main() {
    ...
    if args.get {
        ...
        for i in &days {
            ...
            let mut input_file = OpenOptions::new()
                ...
                .truncate(true)
                .create(true)
                .open(format!("days/day-{:02}/input.txt", i))
            ...
            let session = match args.session {
                ...
                None => std::env::var("AOC_SESSION")
                    .expect("Session key was not found in program args or env"),
            };

            input = match fetch_puzzle_input(args.year, *i, &session) {
                Ok(s) => s.clone(),
                Err(err) => {
                    error!("Failed to fetch puzzle input: {err}");
                    continue;
                }
            };
    ...
    for i in &days {
        ...
        match args.part {
            ...
            0 => {
                println!("Part 1: {:?}", aoc_2023_rust_lib::solve_part(*i, 1));
                println!("Part 2: {:#?}", aoc_2023_rust_lib::solve_part(*i, 2))
            }
            _ => {
                error!("Unknown part!");
                exit(-1)
