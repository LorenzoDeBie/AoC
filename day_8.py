...
input = '''bgafcde gfcd agc ebdgac adfceb bafeg efgca cgdfae cg ecadf | fabgced gc agc cdfg
...
cgefb cbe cfab afdbeg cb faebg gcfde cefbag ebadgc afdbgec | egbdcfa cbe cafb fecagbd'''.splitlines()
...
#input = '''acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab|cdfeb fcadb cdfeb cdbaf'''.splitlines()
...
def main():
    ...
    for line in input:
        ...
        total += sum([1 for x in digits if len(x) in [2, 3, 4, 7]])
    print(total)


def remove_possibility(segment_map, c, pattern):
    segment_map[c] = [c for c in segment_map[c] if c in pattern]

def remove_possibility(segment_map, pattern, keep):
    for c in segment_map:
        if c in keep:
            segment_map[c] = [x for x in segment_map[c] if x in pattern]
        else:
            segment_map[c] = [x for x in segment_map[c] if x not in list(pattern)] 
...
def main2():
    ...
    for line in input:
        ...
        segment_map = {x: list('abcdefg') for x in 'abcdefg'}
        print(segment_map)
        patterns, digits = [x.split() for x in line.split('|')]
        patterns_sorted = [''.join(sorted(x)) for x in patterns]
        print(patterns_sorted, digits)
        for digit in digits:
            if ''.join(sorted(digit)) not in patterns_sorted:
                raise LookupError(f'Unseen pattern in digits! {digit}')
        for pattern in patterns_sorted + digits:
            print(pattern)
                # found a 1
            if len(pattern) == 7:
                continue
            elif len(pattern) == 2:
                remove_possibility(segment_map, pattern, 'cf')
            elif len(pattern) == 3:
                remove_possibility(segment_map, pattern, 'acf')
            elif len(pattern) == 4:
                remove_possibility(segment_map, pattern, 'bdcf')
            elif len(pattern) == 6:
                # find missing letter
                missing = [c for c in 'abcdefg' if c not in pattern][0]
                print(missing)
                # missing is c, d or e -> remove missing from a, b, f, g
                for c in segment_map:
                    if c not in 'cde':
                        segment_map[c] = [x for x in segment_map[c] if x != missing] 
            pprint(segment_map)
            # check for solved segments
            for s, poss in segment_map.items():
                if len(poss) == 1:
                    remove_possibility(segment_map, poss[0], s)
            pprint(segment_map)
        transmap = {}
        for c in segment_map:
            if len(segment_map[c]) != 1:
                raise ValueError('Need to do more calcs!')
            segment_map[c] = segment_map[c][0]
            transmap[ord(c)] = ord(segment_map[c][0])
        transmap = {v: k for k, v in transmap.items()}
        print(segment_map)
        numbersmap = {
            'abcefg': '0',
            'cf': '1',
        ...
        number = ''
        for digit in digits:
            original = digit.translate(transmap)
            original_sorted = ''.join(sorted(list(original)))
            print(digit, '=', original, '->', original_sorted, '=', numbersmap[original_sorted])
            number += numbersmap[original_sorted]
        total += int(number)
    print(total)
